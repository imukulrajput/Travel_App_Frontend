    ...
    (async () => {
      try {
        const { data } = await axios.get(
          `https://travel-app-8a1c.onrender.com/api/hotels/${id}`
        );
        setSingleHotel(data);
      } catch (err) {
        console.log(err);
...
const handleConfrimBookingClick = async () => {
    ...
    <Fragment>
      ...
      <main className="payment-page d-flex justify-center">
        <div className="final-details-container d-flex direction-column gap-md">
          ...
          <button className="button btn-primary btn-reserve cursor btn-pay" onClick={handleConfrimBookingClick}>
            Confirm Booking
          </button>
        </div>
